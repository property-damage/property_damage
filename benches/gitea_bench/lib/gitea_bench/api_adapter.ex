defmodule GiteaBench.ApiAdapter do
  ...
  Each `execute/2` performs the mutation via REST (acting as the relevant user)
  ...
  @impl true
  def execute(%CreateUser{login: login, email: email}, %{client: client}) do
  ...
  def execute(%CreateIssue{repo: full_name, title: title}, %{client: client}) do
  ...
  def execute(%CreateLabel{repo: full_name, name: name, color: color}, %{client: client}) do
    {owner, repo} = GiteaBench.split_full_name(full_name)
  ...
  def execute(%CloseIssue{target: %{repo: full_name, number: number}}, %{client: client}) do
    {owner, repo} = GiteaBench.split_full_name(full_name)
