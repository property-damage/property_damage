...
Defines the two-phase execution model, adapter lifecycle, symbolic reference resolution, external field markers, event injection, and mock service support that together form the core runtime of the PropertyDamage SPBT framework.

Reference DRs: DR-010 (Symbolic References), DR-011 (External Field Markers), DR-015 (Adapter Separation), DR-016 (Injector Pattern), DR-018 (Resource Polling)

## Requirements

### Requirement: Two-Phase Execution

The system SHALL execute command sequences in two distinct phases: a symbolic generation phase that produces command sequences without contacting the SUT, followed by a concrete execution phase that runs commands against the SUT and resolves symbolic values to real ones.

#### Scenario: Symbolic phase generates commands without SUT interaction
- **WHEN** the framework generates a command sequence
- **THEN** all commands SHALL contain symbolic placeholders for server-generated values
...
- **AND** symbolic references SHALL be replaced with concrete values returned by the SUT
...
### Requirement: Symbolic Reference Resolution

The system SHALL use `make_ref/0`-based symbolic references to link command outputs to future command inputs. During concrete execution, symbolic references in command fields SHALL be replaced with concrete values from corresponding event fields.

#### Scenario: Reference creation during symbolic phase
- **WHEN** a command is generated that will produce an entity
- **THEN** a symbolic reference wrapping `make_ref/0` SHALL be created
- **AND** the reference SHALL have no concrete value until execution

#### Scenario: Reference resolution during concrete phase
- **WHEN** a command containing a symbolic reference is about to execute
- **THEN** the framework SHALL replace the symbolic reference with its resolved concrete value
- **AND** if the reference has not yet been resolved, execution SHALL fail

#### Scenario: Reference identity
- **WHEN** two references are compared
- **THEN** identity SHALL be determined by the underlying `make_ref/0` value
- **AND** the label field SHALL NOT affect identity

### Requirement: External Field Markers

The system SHALL support `external()` markers in event struct definitions to identify fields whose values are generated by the SUT. External fields SHALL be automatically detected and populated during concrete execution.
...
- **AND** the concrete value SHALL be captured from the adapter's returned event
